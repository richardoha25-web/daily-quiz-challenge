...
function App() {
  ...
  const start = async (c: number) => {
    setCat(c); setQuestions([]); setIdx(0); setScore(0); setTime(15); setPicked(null); setRewardClaimed(false); setQuizLoading(true); setScreen('quiz');
    try {
      const selected = await getQuizQuestions(all[c].name, all[c].questions, 10);
      const mapped: Q[] = selected.map((q) => ({ q: q.question, a: q.correctAnswer, o: q.options }));
      if (mapped.length >= 10) setQuestions(mapped.slice(0, 10));
      else setQuestions([...all[c].questions].sort(() => Math.random() - 0.5).slice(0, 10));
    } catch {
  ...
  useEffect(() => {
    if (screen !== 'quiz' || picked || quizLoading || !questions.length) return;
    const t = setInterval(() => setTime((x) => { if (x <= 1) { clearInterval(t); answer(''); return 0; } return x - 1; }), 1000);
    return () => clearInterval(t);
  }, [screen, idx, picked, quizLoading, questions.length]);

  const answer = (v: string) => {
    if (picked || quizLoading || !questions[idx]) return;
  ...
  const reset = () => { setScreen('home'); setPicked(null); setQuizLoading(false); setRewardClaimed(false); };
  ...
  return (
    <div className="app">
      ...
      {screen === 'home' && <main>
        ...
        <div className="grid">{all.map((c, i) => <button className="cat" key={c.name} onClick={() => void start(i)}><span>{c.icon}</span><div><b>{c.name}</b><small>{c.questions.length} local questions</small></div><strong>›</strong></button>)}</div>
      ...
      {screen === 'quiz' && <main>
        ...
        {quizLoading ? <section className="question"><div className="qcat">{all[cat].icon} {all[cat].name}</div><h1>Preparing your quiz…</h1><p>Checking your question cache and online question source.</p></section> : <><div className="timer">{time}s</div>
      ...
      {screen === 'result' && <main className="result">
        ...
        <button className="primary" onClick={() => void start(cat)}>Play Again</button><button className="secondary" onClick={reset}>Choose Another Category</button>
      </main>}
      <footer>Daily Quiz & Challenge · Version 1.1 · {bank.length} local questions in this category · Ads help keep the quiz free.</footer>
    </div>
  );
}
